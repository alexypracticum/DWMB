...
- **Журнал событий** (`/admin/event-log`): аудит-журнал всех изменений с фильтрами по типу/сущности/дате/пользователю, пагинация
- **Управление ролями** (`/admin/roles`): CRUD ролей, назначение разрешений, защита от удаления ролей с пользователями
- **Настройки API ключей** (`/admin/api-settings`): OMDB, Last.fm, TMDB, AI ключи с валидацией и статусом source
- **Настройки email** (`/admin/email-settings`): SMTP host/port/user/password
- **Настройки безопасности** (`/admin/security`): CORS origins, rate limiting, CSRF toggle, статус SECRET_KEY
- **Бэкап/восстановление** (`/admin/backup`): создание, скачивание, восстановление, удаление бэкапов PostgreSQL
- **Модель AppSetting**: таблица `meta.app_setting` для хранения конфигурации в БД (SQL миграция 014)
...
- **Dark mode**: удалён дублирующийся маршрут из main.py, переписан toggle для корректного переключения между системными темами, добавлен сброс кэша
- **Event log**: исправлен Jinja2 синтаксис в шаблоне (truncate вместо среза)
