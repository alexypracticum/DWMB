...
# Create a separate sync engine for GraphQL queries
...
class Query:
    ...
    async def stats(self) -> Stats:
        ...
        try:
            entity_count = session.scalar(select(func.count(Entity.entity_id)))
            kind_count = session.scalar(
                select(func.count(EntityKind.kind_id)).where(EntityKind.is_abstract == False)
            )
            relation_count = session.scalar(select(func.count(SemanticRelation.relation_id)))
            model_count = session.scalar(select(func.count(OntologyModel.model_id)))
    ...
    async def models(self) -> List[OntologyModelType]:
        ...
        session = get_sync_session()
        try:
            result = session.execute(select(OntologyModel))
            return [
                OntologyModelType(
    ...
    async def relation_types(self) -> List[RelationTypeType]:
        ...
        session = get_sync_session()
        try:
            result = session.execute(select(RelationType))
            return [
                RelationTypeType(
